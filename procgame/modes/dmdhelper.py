...
class DMDHelper(Mode):
    ...
    def yamlBasedLayerGenerator(self, yamlSect):
        if('display' in yamlSect ):
            yamlSect = yamlSect['display']
            return self.yamlBasedLayerGenerator(yamlSect)
        elif('sequence' in yamlSect):
            v = yamlSect['sequence']
            ...
            for c in v['contents']:
                ...
                c = c['item']
                l = self.yamlBasedLayerGenerator(c)
            ...
            sl.hold = not repeat
            return sl
        elif('group' in yamlSect):
            v = yamlSect['group']
            x = value_for_key(v,'x',0)
            y = value_for_key(v,'y',0)
            w = value_for_key(v,'width',self.game.dmd.width)
            h = value_for_key(v,'height',self.game.dmd.height)
            ...
            for c in contents:
                l = self.yamlBasedLayerGenerator(c)
                lyrs.append(l)
            gl = dmd.GroupedLayer(w, h, lyrs)
            gl.set_target_position(x, y)
            return gl
        elif('animation' in yamlSect):
            v = yamlSect['animation']
            x = value_for_key(v,'x',0)
            y = value_for_key(v,'y',0)
            repeat = value_for_key(v, 'repeat', None)
            lyrTmp = self.game.animations[value_for_key(v,'name')]
            if(repeat is not None):
                lyrTmp.repeat = repeat
            lyrTmp.reset()
            lyrTmp.set_target_position(x, y)
            return lyrTmp
        elif ('text' in yamlSect):
            print(yamlSect)
            v = yamlSect['text']
            fsV = value_for_key(v, 'font_style')
            font_style=None
            if(fsV is not None):
                try:
                    font_style=self.game.fontstyles[fsV]
                except KeyError, e:
                    logging.error("the specified font style '%s' was not defined in the machine yaml file." % fsV)

            txt = value_for_key(v,'msg') 
            font = value_for_key(v,'font')
            if(font is not None):
                try:
                    font=self.game.fonts[font]
                except KeyError, e:
                    logging.error("the specified font '%s' was not defined in the machine yaml file.  Using default 'med' instead" % font)
                    font = self.game.fonts["med"]

            x = value_for_key(v,'x') 
            y = value_for_key(v,'y')
            justify = value_for_key(v,'h_justify')
            v_justify = value_for_key(v,'v_justify')
            w = value_for_key(v,'width',self.game.dmd.width)
            h = value_for_key(v,'height',self.game.dmd.height)
            fill_color = value_for_key(v,'fill_color',(0,0,0))

            l = HDTextLayer(x, y, font, justify=justify, vert_justify=v_justify, opaque=False, width=w, height=h, line_color=None, line_width=0, interior_color=(255,255,255), fill_color=fill_color)
            l.set_textTL(txt,style=font_style)            
            return l
        else:
            raise ValueError, "Unknown tag '%s' in yaml section" % yamlSect
